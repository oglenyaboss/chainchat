...
function saveDifficulty(): void {
  const val = Number.parseInt(difficulty.value, 10)
  blockchainStore.difficulty = Math.max(1, Math.min(6, Number.isNaN(val) ? 2 : val))
}
...
<template>
  <div class="settings">
    <Win95Tabs v-model="activeTab" :tabs="tabs">
      <!-- Identity Tab -->
      ...
      <div v-if="activeTab === 'mining'" class="settings__tab">
        ...
        <p class="settings__hint">
          Higher = slower blocks. Use 1-2 for demo, 3-4 for realistic feel.
        </p>
      </div>
      ...
      <div v-if="activeTab === 'danger'" class="settings__tab">
        <Win95GroupBox label="Danger Zone">
          ...
          <Win95Button @click="resetIdentity">
